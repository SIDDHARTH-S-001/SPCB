...
void loop() {
   ...
   distance = duration / 58; // Speed of sound in air is 343 m/s or 0.0343 cm/µsV,JU
  Serial.print("Distance: ");
  Serial.print(distance);
  Serial.println(" cm");
  
  // Check if the distance is greater than 30 cm
  if (distance > 30) {
...
void loop() {
   ...
   pinMode(echoPin, INPUT);
   duration = pulseIn(echoPin, HIGH);
   distance = duration / 58; // Speed of sound in air is 343 m/s or 0.0343 cm/µs
  Serial.print("Distance: ");
  Serial.print(distance);
  Serial.println(" cm");
